...
void resetGameState(int& t1, double& worldTime, unicorn& unicorn, obstacle& obstacle_1,
	...
	platform& platform_3, platform& platform_4, double& cameraPosition);

void initGameState(int& t1, double& worldTime, unicorn& unicorn, obstacle& obstacle_1,
	obstacle& obstacle_2, obstacle& obstacle_3, platform& platform_1, platform& platform_2,
	platform& platform_3, platform& platform_4, double& cameraPosition, FILE* file);
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < obstacleCount; i++) {
		fgets(line, 128, file);
		setObstacleParameters(obstacles[i], stringToInt(line, 1), stringToInt(line, 2), stringToInt(line, 3));
		printf("%d %d % d\n", stringToInt(line, 1), stringToInt(line, 2), stringToInt(line, 3));
	}
	...
	for (int i = 0; i < platformCount; i++) {
		fgets(line, 128, file);
		setPlatformParameters(platforms[i], stringToInt(line, 1), stringToInt(line, 2), stringToInt(line, 3));
		printf("%d %d % d\n", stringToInt(line, 1), stringToInt(line, 2), stringToInt(line, 3));
	}


	resetGameState(t1, worldTime, unicorn, obstacle_1, obstacle_2, obstacle_3, platform_1, platform_2, platform_3, platform_4, cameraPosition);
	...
	while (!(quit)) {
		...
		//sprawdzenie czy jednoro¿ec nie spad³
			...
			case SDL_KEYDOWN:
				if (event.key.keysym.sym == SDLK_ESCAPE) quit = 1;
					...
					if (!unicorn.isOnDash && unicorn.jumpState < 2) {
						...
						unicorn.position.y -= 5;
						unicorn.velocity.y = -1.5;
					}
				}
				...
				else if (event.key.keysym.sym == SDLK_n) resetGameState(t1, worldTime, unicorn, obstacle_1, obstacle_2, obstacle_3, platform_1, platform_2, platform_3, platform_4, cameraPosition);
...
void resetGameState(int& t1, double& worldTime, unicorn& unicorn,
	...
	platform& platform_4, double& cameraPosition) {

	t1 = SDL_GetTicks();

	worldTime = 0;
	unicorn.velocity.x = 2;
	...
	setObstacleParameters(obstacle_1, 280, 160, 0);
	setObstacleParameters(obstacle_2, 200, 80, 200);
	setObstacleParameters(obstacle_3, 80, 180, double(FRAME_THICKNESS - ADDITIONAL_SCREEN_HEIGHT));

	setPlatformParameters(platform_1, 1280, 250, double(SCREEN_HEIGHT - FRAME_THICKNESS - 250));
	setPlatformParameters(platform_2, 1000, 120, double(SCREEN_HEIGHT - FRAME_THICKNESS - 160));
	setPlatformParameters(platform_3, 650, 70, double(platform_2.position.y - unicorn.height - 150));
	setPlatformParameters(platform_4, 650, 70, double(SCREEN_HEIGHT - FRAME_THICKNESS - 450));
...
void update(unicorn& unicorn, obstacle& obstacle_1, obstacle& obstacle_2,
	...
	if (unicorn.isOnRun) {
		...
		//fizyka poruszania siê, skoków i dashów

		if (unicorn.jumpState > 0) {
			unicorn.velocity.y += GRAVITY;
		}
		else {
			unicorn.velocity.y += 0.1 * GRAVITY;
		}
		...
		if (unicorn.isOnDash) {
			unicorn.position.x += 3 * unicorn.velocity.x;
		...
		//prêdkoœæ zwiêkszana wraz z przyrostem czasu
		unicorn.velocity.x += delta / 10;
		...
		if (!unicorn.isOnDash) {
			...
			//kontrola powierzchi, na której znajduje siê jednoro¿ec
...
void render(SDL_Surface* screen, unicorn& unicorn, obstacle& obstacle_1, obstacle& obstacle_2, obstacle& obstacle_3,
	...
	//rysowanie obramowania planszy, gruntu i wyœwietlacza czasu
	drawLayout(screen, charset, worldTime);

	//rysowanie jednoro¿ca
...
void generateObstaclesAndPlatfroms(unicorn& unicorn, obstacle& obstacle_1, obstacle& obstacle_2, obstacle& obstacle_3,
	...
	for (int i = (unicorn.position.x - 1280); i < (unicorn.position.x + SCREEN_WIDTH); i++) {
		...
		if (i % (obstacle_3.width * 16) == 0 && i != 0 &&
...
void drawLayout(SDL_Surface* screen, SDL_Surface* charset, double& worldTime) {
	...
	//wyœwietlanie czasu gry w sekundach w prawym górnym rogu ekranu
	char text[128];
...
void drawUnicorn(SDL_Surface* screen, unicorn& unicorn) {
	...
	//oznaczenie koloru czarnego jako niewidoczny i narysowanie jednoro¿ca
...
bool isPlatformCollision(unicorn& unicorn, platform& platform, double& cameraPosition) {
	...
	if (platform.isOnScreen) {
		if ((unicorn.position.x + unicorn.width) >= platform.position.x &&
			...
			(unicorn.position.y + unicorn.height) > (platform.position.y + 10 - cameraPosition) &&
